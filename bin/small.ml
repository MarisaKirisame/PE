...
let rec evalAux(e: value env): term -> value =
  ...
  | FromVar (Var v) -> e v
  | Abs (Var v, b) ->
    (VFun (fun p -> evalAux (extend e v p) b))
  ...
  | Match (s, lcase, rcase) ->
    ...
    match ps with
     | (VSum (Left x)) -> app pl x
     | (VSum (Right x)) -> app pr x
...
let rec rEvalAux(curStore: rValue env ref)(e: rValue env): term -> rValue =
  ...
  | FromVar (Var v) -> e v
  | Abs (Var v, b) ->
    (RFun (fun p -> rEvalAux curStore (extend e v p) b))
  | App (f, x) -> app (recurse f) (recurse x)
  | Unit -> RUnit
  | Int f -> RInt f
  | Add (x, y) ->
  ...
  | SetRef (r, v) ->
    ...
    let rv = recurse v in
    (match rr with
     | RRef (StoreId s) -> curStore := extend (!curStore) s rv);
    RUnit
  | GetRef r ->
    (match recurse r with
     | RRef (StoreId s) -> (!curStore) s)
  ...
  | Match (s, lcase, rcase) ->
    ...
    match rs with
     | (RSum (Left x)) -> app rl x
     | (RSum (Right x)) -> app rr x
...
let rec anfAux(l : letList): term -> term =
  ...
  | Match (s, lcase, rcase) -> push l (Match (recurse s, recurse lcase, recurse rcase))
